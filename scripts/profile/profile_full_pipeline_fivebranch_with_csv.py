...
def metric_mean_value(errors, metric_index):
    """
    Extract the per-sequence mean value for one metric.
    ...
    The evaluator usually returns [5, 2], where column 0 is the mean
    and column 1 is the standard deviation for the current sequence.
    This helper keeps the CSV values as one mean value per sequence.
    """
    value = errors[metric_index]
...
def save_sequence_metrics_csv(rows, csv_path):
    """
    Save one row per sequence.

    This CSV is used to generate the ECDF plots for the paper:
        Fig. 5: offline mesh error CDF
        Fig. 6: online jitter CDF
    """
    import csv
    ...
    if len(rows) == 0:
        print("[warning] No sequence metrics were collected. CSV was not saved.")
        return
    ...
    print(f"\nSaved per-sequence metrics CSV: {csv_path}")
...
def main():
    ...
    parser.add_argument(
        "--save_sequence_metrics_csv",
        type=str,
        default=None,
        help="Optional path to save one row of evaluation metrics per sequence.",
    )
        ...
        raw_items = [
            {
                ...
                "pose": pose,
                "tran": tran,
                "__sequence_index": index,
                "__source_path": os.path.join(args.dataset_dir, "test.pt") + f"::{index}",
            }
            for index, (acc, ori, pose, tran) in enumerate(
                zip(data["acc"], data["ori"], data["pose"], data["tran"])
            )
        ]
    elif args.raw_list_file is not None:
        raw_items = []
        for index, path in enumerate(load_list(args.raw_list_file)):
            item = torch.load(path, map_location="cpu", weights_only=False)
            if not isinstance(item, dict):
                raise RuntimeError(f"Expected a dictionary in raw sequence file: {path}")
            item = dict(item)
            item["__sequence_index"] = index
            item["__source_path"] = path
    ...
    for sequence_index, raw in enumerate(iterator):
        ...
        offline_orig_error = evaluator.eval(pose_orig_off, pose_t)
        offline_orig_errs.append(offline_orig_error)
        ...
        offline_dist_error = evaluator.eval(pose_dist_off, pose_t)
        offline_dist_errs.append(offline_dist_error)
        offline_dist_times.append(elapsed)
        offline_dist_frames.append(frame_count)

        sequence_row = {
            "sequence_index": int(raw.get("__sequence_index", sequence_index)),
            "source_path": str(raw.get("__source_path", f"sequence_{sequence_index}")),
            "frames": int(frame_count),

            "offline_original_sip_deg": metric_mean_value(offline_orig_error, 0),
            "offline_original_angular_deg": metric_mean_value(offline_orig_error, 1),
            "offline_original_position_cm": metric_mean_value(offline_orig_error, 2),
            "offline_original_mesh_cm": metric_mean_value(offline_orig_error, 3),
            "offline_original_jitter": metric_mean_value(offline_orig_error, 4),

            "offline_distributed_sip_deg": metric_mean_value(offline_dist_error, 0),
            "offline_distributed_angular_deg": metric_mean_value(offline_dist_error, 1),
            "offline_distributed_position_cm": metric_mean_value(offline_dist_error, 2),
            "offline_distributed_mesh_cm": metric_mean_value(offline_dist_error, 3),
            "offline_distributed_jitter": metric_mean_value(offline_dist_error, 4),
        }
        ...
        if args.evaluate_online:
            ...
            online_orig_error = evaluator.eval(pose_orig_on, pose_t)
            online_orig_errs.append(online_orig_error)
            ...
            online_dist_error = evaluator.eval(pose_dist_on, pose_t)
            online_dist_errs.append(online_dist_error)
            ...
            sequence_row.update({
                "online_original_sip_deg": metric_mean_value(online_orig_error, 0),
                "online_original_angular_deg": metric_mean_value(online_orig_error, 1),
                "online_original_position_cm": metric_mean_value(online_orig_error, 2),
                "online_original_mesh_cm": metric_mean_value(online_orig_error, 3),
                "online_original_jitter": metric_mean_value(online_orig_error, 4),

                "online_distributed_sip_deg": metric_mean_value(online_dist_error, 0),
                "online_distributed_angular_deg": metric_mean_value(online_dist_error, 1),
                "online_distributed_position_cm": metric_mean_value(online_dist_error, 2),
                "online_distributed_mesh_cm": metric_mean_value(online_dist_error, 3),
                "online_distributed_jitter": metric_mean_value(online_dist_error, 4),
            })

        sequence_metric_rows.append(sequence_row)
    ...
    save_sequence_metrics_csv(sequence_metric_rows, args.save_sequence_metrics_csv)
