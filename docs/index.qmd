...
```{mermaid}
flowchart TD
    IMU["Six sparse IMUs"] --> ROOT["Shared root reference"]
    ROOT --> LL["Left-leg branch"]
    ROOT --> RL["Right-leg branch"]
    ROOT --> TH["Trunk-head branch"]
    ROOT --> LA["Left-arm branch"]
    ROOT --> RA["Right-arm branch"]
    LL & RL & TH & LA & RA --> S1["Pose-S1<br/>distributed leaf-joint position estimation"]
    S1 --> S2["Pose-S2<br/>distributed full-joint position estimation"]
    S2 --> S3["Pose-S3<br/>distributed reduced-joint rotation estimation"]
    S3 --> FUSION["Learned residual rotation fusion"]
    FUSION --> POSE["Full SMPL pose"]
```
